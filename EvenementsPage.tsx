...
import { Calendar, MapPin, Clock, ArrowRight, Filter, CalendarDays } from "lucide-react";
...
const EvenementsPage = () => {
  ...
  useEffect(() => {
    const fetchEvents = async () => {
      try {
        ...
        // Backend renvoie directement un tableau d'événements avec event_prices
  ...
  // Filtres dynamiques basés sur les types d'événements
    ...
    <main className="min-h-screen">
      ...
      <section className="pt-32 pb-20 bg-[hsl(220,63%,16%)] relative overflow-hidden">
        <div className="container-custom relative z-10 max-w-3xl">
          <div className="inline-flex items-center gap-2 bg-accent/20 text-accent px-4 py-2 rounded-full mb-6">
            ...
            <span className="text-sm font-medium">{t("evenementsPage.hero.badge")}</span>
          </div>
          <h1 className="font-heading text-5xl font-bold text-white mb-6">
            {t("evenementsPage.hero.title")}
          </h1>
          <p className="text-white/80 text-lg">{t("evenementsPage.hero.subtitle")}</p>
        </div>
      </section>
      ...
      <section className="section-padding">
        <div className="container-custom">
          {loading && <p className="text-center text-muted-foreground">Chargement des événements...</p>}
          ...
          {!loading && filteredEvents.length === 0 && (
            <p className="text-center text-muted-foreground mt-20">Aucun événement trouvé</p>
          )}
        </div>
      </section>
      ...
      <section className="py-20 bg-secondary">
        <div className="container-custom text-center">
          <h2 className="text-4xl font-bold mb-6">{t("evenementsPage.cta.title")}</h2>
          <p className="text-muted-foreground max-w-2xl mx-auto mb-8">{t("evenementsPage.cta.subtitle")}</p>
          <Button variant="accent" size="xl" asChild>
            <a href="/contact">{t("evenementsPage.cta.button")}</a>
          </Button>
        </div>
      </section>
